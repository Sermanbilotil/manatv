...
const nextButtonComponent = (videoPlayer, callback) => {
  ...
  svgContainer.innerHTML = `<img src="${nextPlayIcon}" />` 
...
const seasonComponent = ({videoPlayer, videojs, seriesData, titleSeason, titleSeries, seasonPrev, seasonNext}) => {
  ...
  controlText.innerHTML = titleSeries ?? ''

  const svgContainer = document.createElement('div')
  svgContainer.className = 'svg-container';
  svgContainer.innerHTML = `<img src="${playlistPlayerIcon}" />` 
  ...
  menuTitle.innerHTML = titleSeason || 'Season 1'
  ...
  if (seasonPrev?.title && seasonPrev?.onClick) {
    ...
    changeSeasonPrev.innerHTML = `<span>${seasonPrev.title}</span>`
  ...
  if (seasonNext?.title && seasonNext?.onClick) {
    ...
    changeSeasonNext.innerHTML = `<span>${seasonNext.title}</span>`
